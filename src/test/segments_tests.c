...
char *test_segments_should_maintain_order_and_be_monotonic() {
  ...
  mu_assert(segments_add(&s, 4) == OK, "failed to add");
  mu_assert(segments_add(&s, 1) == OK, "failed to add");
  mu_assert(segments_add(&s, 3) == OK, "failed to add");
  mu_assert(segments_add(&s, 2) == OK, "failed to add");
